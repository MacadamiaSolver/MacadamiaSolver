...
let test_exists_bigger () =
  Alcotest.(check string)
    ...
    ( "AxEy x = y + 1" |> Parser.parse_formula |> Result.get_ok
...
let test_no_biggest_int () =
  Alcotest.(check string)
    ...
    ( "~ExAyEz(x = y + z)" |> Parser.parse_formula |> Result.get_ok
    |> Ast.string_of_formula )
...
let test_divisible_by_7 () =
  Alcotest.(check string)
    ...
    ( "Ex y = x + x + x + x + x + x + x" |> Parser.parse_formula
    |> Result.get_ok |> Ast.string_of_formula )
...
let test_sum_of_evens_is_even () =
  Alcotest.(check string)
    ...
    |> Parser.parse_formula |> Result.get_ok |> Ast.string_of_formula )
*)
let tests =
  ( "Parser"
