...
let dump f = match eval f with
  | Ok nfa -> Format.asprintf "%a" (Nfa.format_nfa format_atom) nfa |> Result.ok
  | Error _ as e -> e

let list () =
  let rec aux = function
    | [] -> ()
    | (name, params, f, _) :: xs -> Format.printf "%s %a = %a\n%!" name (Format.pp_print_list Format.pp_print_string) params Ast.pp_formula f; aux xs in
  aux !state

let pred name params f =
  match eval f with
  | Ok nfa -> state := (name, params, f, nfa) :: !state; Result.ok ()
  | Error _ as e -> e

let proof f = match eval f with
  | Ok nfa -> Nfa.run_nfa nfa [] |> Result.ok
  | Error _ as e -> e
