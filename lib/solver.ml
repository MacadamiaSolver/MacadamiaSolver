...
type t = { preds: (string * string list * Ast.formula * Nfa.t) list
...
let collect f = 
  ...
  let rec collect_formula = function
    ...
    | Ast.Pred (_, args) ->
        List.fold_left (fun acc x -> Set.union acc (collect_term x)) Set.empty args
    | _ ->
        Set.empty in
  collect_formula f
  |> Set.to_list
...
let progress s =
  ...
  let c =
    Float.of_int s.progress /. Float.of_int s.total *. Float.of_int pl |> Int.of_float
  in
...
let teval s ast =
  ...
  let internal () =
    ...
    !internal_counter - 1 + Map.length s.vars in
    ...
    let nfa =
      match ast with
        ...
        | Ast.Geq (l, r) ->
            ...
            |> Nfa.intersect ra
            |> Nfa.truncate (deg ())
            |> Nfa.to_dfa |> Nfa.minimize |> Nfa.to_nfa
            |> return
        | Ast.Mnot f ->
          let* nfa = eval f in
        ...
        | Ast.Exists (x, f) ->
          let* nfa = eval f in
          let x = var_exn x in
          nfa |> Nfa.project [x] |> Nfa.to_dfa |> Nfa.minimize |> Nfa.to_nfa |> return
        | Ast.Any (x, f) ->
          let* nfa = eval f in
